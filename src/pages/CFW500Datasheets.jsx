...
import drives, { pdfPath, partNumber } from '../data/cfw500Data'

const manuals = [
  { tag: 'CFW500', label: "User's Manual",             url: '/datasheets/CFW500/WEG-CFW500-USERS-MANUAL.pdf' },
  { tag: 'CFW500', label: 'Control Brochure',          url: '/datasheets/CFW500/WEG-CFW500-CONTROL-BROCHURE.pdf' },
  { tag: 'CFW500', label: 'Quick Parameter Reference', url: '/datasheets/CFW500/WEG-CFW500-QUICK-PARAMETER-REFERENCE.pdf' },
]

const websiteLinks = [
  { tag: 'CFW500', label: 'CFW500 Product Page', url: 'https://pamensky.com/cfw500.html' },
]

const tagBadge = {
  'CFW500': 'bg-amber-900/40 text-amber-300 border-amber-700/40',
}
...
const allVoltage = [ALL, '460V', '230V', '575V']
const allSeries  = [ALL, 'Standard', 'NEMA 4X', 'NEMA 4X + Disc']

const voltageInfo = {
  '460V': { label: '460V',  badge: 'bg-blue-900/40 text-blue-300 border-blue-700/40' },
  '230V': { label: '230V',  badge: 'bg-green-900/40 text-green-300 border-green-700/40' },
  '575V': { label: '575V',  badge: 'bg-amber-900/40 text-amber-300 border-amber-700/40' },
}

const seriesInfo = {
  'Standard':      { badge: 'bg-slate-800/60 text-slate-300 border-slate-600/40' },
  'NEMA 4X':       { badge: 'bg-indigo-900/40 text-indigo-300 border-indigo-700/40' },
  'NEMA 4X + Disc':{ badge: 'bg-teal-900/40 text-teal-300 border-teal-700/40' },
}
...
const voltageOrder = { '460V': 0, '230V': 1, '575V': 2 }
const seriesOrder  = { 'Standard': 0, 'NEMA 4X': 1, 'NEMA 4X + Disc': 2 }

export default function CFW500Datasheets() {
  ...
  const hasFilters = voltage !== ALL || series !== ALL || hp !== ALL

  function clearAll() { setVoltage(ALL); setSeries(ALL); setHp(ALL) }
      ...
      <div className="bg-[#080808] border-b border-white/5 py-14 relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-6">
          ...
          <p className="text-gray-400 max-w-xl">
            Filter by enclosure series, voltage class, and horsepower to find and download the correct WEG CFW500 VFD datapack.
          </p>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-6 py-10">
        ...
        <div className="bg-[#0d0d0d] border border-white/8 p-6 mb-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {/* Series */}
            <div>
              <div className="text-xs text-gray-500 uppercase tracking-widest mb-2">Enclosure Series</div>
                ...
                {pageItems.map((d, i) => (
                  ...
                  >
                    <td className="px-4 py-3">
                      <span className={`inline-block text-xs font-semibold px-2 py-0.5 border ${seriesInfo[d.series].badge}`}>
                        {d.series}
                      </span>
                    </td>
                    <td className="px-4 py-3">
